...
export async function getZipcodeBorders() {
  let zipCodeJson = require("./assets/zip_code.json");
  // zipCodeJson = [...zipCodeJson.features];
  zipCodeJson = zipCodeJson.features;
  const zipCodeCoordinates = zipCodeJson.map((entry) => ({
    zipCode: entry.properties.ZIPCODE,
    coordinates: entry.geometry.coordinates[0].map((e) => ({
      latitude: Number(e[1]),
